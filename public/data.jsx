...
window.SEED = {
  ...
  kpis: [
    { id: 'm1', label: 'Activation', source: 'snowflake', sql: 'SELECT activation_rate AS value, activation_prior AS prev FROM analytics.kpis.activation', format: '%', value: '31.4%', target: '+15% by EOQ', timeframe: '12w', trend: { dir: 'up',   pct: 2.1, period: 'vs last week' }, series: [27.8, 28.1, 28.9, 29.4, 29.1, 29.8, 30.2, 30.0, 30.7, 31.0, 31.2, 31.4], seriesLabels: ['12 wk ago', '', '', '', '', '', '', '', '', '', '', 'now'] },
    { id: 'm2', label: 'Weekly active learners', source: 'snowflake', sql: 'SELECT wal_current AS value, wal_prior AS prev FROM analytics.kpis.weekly_active_learners', format: 'k', value: '412k', target: 'target 450k', timeframe: '12w', trend: { dir: 'up',   pct: 4.8, period: 'vs last week' }, series: [372, 380, 385, 388, 390, 392, 398, 401, 403, 405, 409, 412], seriesLabels: ['12 wk ago', '', '', '', '', '', '', '', '', '', '', 'now'] },
    { id: 'm3', label: 'Time to first lesson', source: 'snowflake', sql: 'SELECT ttfl_hours AS value, ttfl_prior AS prev FROM analytics.kpis.time_to_first_lesson', format: 'h', value: '63h', target: 'target <48h', timeframe: '12w', trend: { dir: 'down', pct: 6,   period: 'improving', good: true }, series: [74, 72, 71, 70, 69, 68, 67, 66, 66, 65, 64, 63], seriesLabels: ['12 wk ago', '', '', '', '', '', '', '', '', '', '', 'now'] },
    { id: 'm4', label: 'NPS', source: 'snowflake', sql: 'SELECT nps_score AS value, nps_prior AS prev FROM analytics.kpis.nps', format: 'plain', value: '57', target: 'target 60+', timeframe: '12w', trend: { dir: 'flat', pct: 0, period: 'no change' }, series: [54, 55, 56, 55, 57, 56, 57, 57, 56, 57, 57, 57], seriesLabels: ['12 wk ago', '', '', '', '', '', '', '', '', '', '', 'now'] },
  ],
